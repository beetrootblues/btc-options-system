    ...
    def format_telegram_message(self, report: dict) -> str:
        ...
        if signals:
            ...
            for i, sig in enumerate(signals, 1):
                ...
                conf_label = sig.get('confidence_label', sig.get('confidence', 'MEDIUM'))
                conf_icon = {'VERY_HIGH': '\U0001f7e2', 'HIGH': '\U0001f7e2', 'MEDIUM': '\U0001f7e1', 'LOW': '\U0001f534'}.get(
                    conf_label, '\u26aa')

                # Build confidence bar [||||      ] 64/100
                filled = int(conf_score / 10)
                bar = '\u2588' * filled + '\u2591' * (10 - filled)

                lines.append(f'{conf_icon} <b>Signal {i}: {sig["strategy_name"]}</b>')
                lines.append(f'  Confidence: [{bar}] {conf_score}/100 ({conf_label})')
                lines.append(f'  Type: {sig["signal_type"]} | Dir: {sig["direction"].upper()}')
                lines.append(f'  {sig["description"]}')

                size_btc = sig['suggested_size_btc']
                lines.append(f'  <b>Size: {size_btc:.4f} BTC (${size_btc * btc_price:,.2f})</b>')
                lines.append(f'  Max Loss: ${sig["max_loss_usd"]:,.2f}')
                lines.append(f'  Expiry: {sig["suggested_expiry"]}')

                # Factor breakdown
                factors = sig.get('sizing_factors', {})
                if factors:
                    lines.append(f'  Factors: IV:{factors.get("iv_environment",0)} | VRP:{factors.get("vrp_edge",0)} | Reg:{factors.get("regime_alignment",0)} | Tech:{factors.get("technical_confluence",0)} | VoV:{factors.get("vov_confirmation",0)} | Theta:{factors.get("theta_profile",0)}')
